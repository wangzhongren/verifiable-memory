...
When the user supplies an explicit correction to a prior branch decision, `policy-learn` can warm-start a new model version with the old training data and feedback. Check the old data digest, preserve both old artifacts, and provide an independent evaluation set when available. Without that evaluation, the new model is a candidate; do not route with it by default or claim a verified improvement. A numerically valid path is not a substitute for a correct semantic label.

When asked for AI self-correction, use `policy-self-study` only with a separate book of source/query/edge answers that can be checked against the current graph. The controller answers first; the system compares with the book, records mistakes as versioned exact-query corrections, and attempts to learn from them. Preserve the book and its hash with the evidence. A rejected candidate model can coexist with an active exact correction. Do not turn the controller's own choice or an ungrounded language-model opinion into a gold label.
