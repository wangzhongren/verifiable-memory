...
两条路径：
- LLM 路径：OpenAI 兼容接口，要求只输出一个 JSON 对象；产出经
...
# paraphrases 子命令用；真实 CLI 运行时传 store.known_slots()。
...
2. 事实内容统一写成"方向是X"的形式（例：方向是向上）。
3. 程序步骤按用户给出的执行顺序排列；"先A再B"对应 ["A","B"]。
4. input 是 2–8 位、每位 0–7 的数字串。
5. 只能解析，不能推断教材之外的语义。无法解析时输出 {"op": "unknown"}。"""
